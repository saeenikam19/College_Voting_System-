...
def get_feedback():
    db = get_db()
    cur = db.cursor()
    cur.execute("SELECT f.id, u.username, f.rating, f.comments, f.timestamp FROM feedback f JOIN users u ON f.user_id = u.id ORDER BY f.timestamp DESC")
    rows = cur.fetchall()
    feedback = []
    for r in rows:
        feedback.append({'id': r['id'], 'username': r['username'], 'rating': r['rating'], 'comments': r['comments'], 'timestamp': r['timestamp']})
    return feedback

def has_user_given_feedback(user_id):
    db = get_db()
    cur = db.cursor()
...
def vote():
    ...
    cands = cur.fetchall()
    return render_template('vote.html', candidates=cands, has_voted=has_voted, has_given_feedback=has_given_feedback)

@app.route('/feedback', methods=['GET','POST'])
def feedback():
    if 'user_id' not in session:
        return redirect(url_for('login'))
    if has_user_given_feedback(session['user_id']):
        flash('You have already submitted feedback. Thank you!', 'info')
        return redirect(url_for('vote'))
    if request.method == 'POST':
        ...
        try:
            db = get_db()
            cur = db.cursor()
            cur.execute("INSERT INTO feedback(user_id, rating, comments, timestamp) VALUES (?,?,?,?)",
                        (session['user_id'], int(rating), comments, datetime.utcnow().isoformat()))
            cur.execute("INSERT INTO audit(action, user, details, timestamp) VALUES (?,?,?,?)",
                        ('feedback', session.get('username'), f'rating={rating}', datetime.utcnow().isoformat()))
            db.commit()
...
def reset_votes():
    ...
    cur.execute("DELETE FROM feedback")  # Reset feedback as well
    cur.execute("UPDATE users SET voted = 0 WHERE role <> 'admin'")
    cur.execute("INSERT INTO audit(action, user, details, timestamp) VALUES (?,?,?,?)",
                ('reset_votes', session.get('username'), 'reset all votes and feedback', datetime.utcnow().isoformat()))
    db.commit()
    try:
        socketio.emit('results_update', get_results(), broadcast=True)
    except Exception:
        app.logger.exception('Socket emit failed on reset_votes')
    flash('All votes and feedback have been reset', 'success')
    return redirect(url_for('admin'))
